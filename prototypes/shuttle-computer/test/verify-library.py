...
BUILD = os.path.join(ROOT, "build", "library")
...
def main():
    ...
    src.append(os.path.join(HERE, "TraxLibraryTests.cs"))
    for s in src:
        if not os.path.isfile(s):
            print("missing source: " + s)
            return 3

    exe = os.path.join(BUILD, "TraxLibraryTests.dll")
    ...
    with io.open(rsp, "w", encoding="utf-8") as f:
        ...
        f.write("-main:TraxLibraryTests" + chr(10))
    ...
    print("compiling the shelf standalone (no Unity references)...")
    ...
    if errors:
        ...
        for l in errors[:20]:
            print("  " + l)
        return 1
    print("  ok - TraxLibrary compiles with zero Unity dependencies")

    cfg = os.path.join(BUILD, "TraxLibraryTests.runtimeconfig.json")
    ...
    print("running the shelf's rules...")
