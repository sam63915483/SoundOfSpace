...
// ── THE ACTIVE SET — appended LAST, deliberately ─────────────────────────────
// Adding cases here rather than mid-list keeps every existing case index put, so
// the golden diff for this change is a pure append and any movement in an older
// vector is impossible to miss. These cases must NOT shift a single voice hash —
// every voice is generated whether or not it is audible — but they MUST shift
// the track id, because a drumless take of the same arrangement is a different
// song and an alien has to be able to tell. Both halves are checked by being here.
for (const m of PRESETS.MODULE_NAMES)
    push (TRACK.setActive (TRACK.defaultTrack (), m, false));
{
    let t = TRACK.defaultTrack ();
    for (const m of PRESETS.MODULE_NAMES) t = TRACK.setActive (t, m, false);
    push (t);                                    // everything muted
    push (TRACK.setActive (t, 'GLOWORM', true)); // bass alone
}
...
const lines = [
    ...
    '# ACTIVE|i|<1 or 0 per module, in META modules order>|mask',
...
CASES.forEach ((t, i) => {
    ...
    lines.push ('ACTIVE|' + i + '|' + PRESETS.MODULE_NAMES.map (m => t.active[m] ? 1 : 0).join ('|') +
                '|' + TRACK.activeMask (t));
