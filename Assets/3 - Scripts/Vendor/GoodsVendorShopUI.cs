...
/// <summary>
/// Procedurally-built shop UI for goods vendors (Alien7, future merchants).
/// Two views in one Canvas: a grid of cards (live-rendered item preview + name + price)
/// and a detail view (bigger preview + description + BUY/BACK). Mirrors the build-everything-
/// in-code pattern from FishMarketNPC and the runtime preview-camera/stage rig from
/// FishingdexManager.SetupPreviewCamera (lines 83–101) / RenderFish (line 285).
/// </summary>
public class GoodsVendorShopUI : MonoBehaviour
{
    // ── Palette (warm copper/amber to differentiate from FishMarket's teal) ──
    ...
    [Tooltip("Auto-created at runtime if null. Lives off-screen, only sees previewLayer.")]
    [SerializeField] Camera previewCamera;
    [SerializeField] Transform previewStage;
    [Tooltip("Layer index used for off-screen item previews. Default 31 (matches FishingdexManager).")]
    ...
    void Update()
    {
        ...
        // F (or controller X) closes the shop. Esc is reserved for the global
        // pause menu — using it here would either pop both the shop and the
        // pause menu at the same time, or eat the Esc press and break pause.
        if (!_shopOpen) return;
        // Defensive cursor pinning — mirrors TutorialPerformanceReview's fix.
        // Without this, the first open after a dialogue ends up with a locked
        // cursor because another script flips CursorLockMode.Locked back on
        // the same frame Open() unlocked it.
    ...
    public void Open(Alien7Vendor vendor)
    {
        if (_shopOpen) return;
        _vendor = vendor;
        if (!_builtUI) BuildUI();
    ...
    void BuildCanvas()
    {
        var canvasGO = new GameObject("GoodsVendorCanvas");
    ...
    void BuildPreviewRig()
    {
        ...
        {
            var camGO = new GameObject("VendorPreviewCamera");
            camGO.transform.SetParent(transform, false);
            // Keep the rig far from world origin so it doesn't intersect terrain.
            camGO.transform.position = new Vector3(0f, -10000f, 0f);
            previewCamera = camGO.AddComponent<Camera>();
            previewCamera.clearFlags = CameraClearFlags.SolidColor;
            previewCamera.backgroundColor = new Color(0f, 0f, 0f, 0f);
            previewCamera.cullingMask = 1 << previewLayer;
            previewCamera.enabled = false; // we drive it via Render() on demand
            previewCamera.allowHDR = false;
            previewCamera.allowMSAA = false;
        }
        if (previewStage == null)
        {
            var stageGO = new GameObject("VendorPreviewStage");
        ...
        // Two lights — key + fill — scoped to the preview layer.
        CreatePreviewLight("VendorPreviewKey",  previewStage.position + new Vector3(-1.5f, 2.5f, -1.5f), 2.5f, 30f, Color.white);
        CreatePreviewLight("VendorPreviewFill", previewStage.position + new Vector3( 2.0f, 0.5f,  1.5f), 1.0f, 25f, new Color(0.85f, 0.9f, 1f));
    }
    ...
    void BuildUI()
    {
        ...
        // Full-screen dim backdrop that swallows clicks behind the panel.
        ...
        // Title bar
        var title = MkText(panel, "GOODS VENDOR", 26, C_Title, FontStyles.Bold, TextAlignmentOptions.Center);
        ...
        // Close hint — uses the live PromptGlyphs glyph so it tracks the
        // current input source (F on KBM, X / Square on controller).
    ...
    void BuildGridView(RectTransform panel)
    {
        ...
        // Scroll viewport (RectMask2D, not Mask — see CLAUDE.md).
        ...
        // Content holder with GridLayoutGroup
    ...
    void PopulateGrid()
    {
        // Wipe old cards (vendor inventory could change between opens in future).
    ...
    {
        bool owned = _vendor != null && item.oneTimePurchase && _vendor.IsAlreadyOwned(item.kind);
        bg.color = owned ? C_Owned : C_CardBg;
        if (raw != null) raw.color = owned ? new Color(1f, 1f, 1f, 0.4f) : Color.white;
        if (price != null)
        {
    ...
    void ShowDetail(ShopItem item)
    {
        ...
        if (_buyLabel != null) _buyLabel.text = owned ? "OWNED" : "BUY";
    ...
    void OnBuyClicked()
    {
        ...
        switch (result)
        {
            case Alien7Vendor.PurchaseResult.Success:
                ShowToast("Purchased!", C_Toast_OK);
                RefreshCardOwnedState(_detailItem);
                // Auto-close so the player sees the equipped item immediately.
                StartCoroutine(CloseAfterDelay(1.0f));
                break;
            case Alien7Vendor.PurchaseResult.NotEnoughMoney:
                ShowToast("Not enough money!", C_Toast_Err);
                break;
            case Alien7Vendor.PurchaseResult.AlreadyOwned:
                ShowToast("Already owned!", C_Toast_Err);
                break;
            case Alien7Vendor.PurchaseResult.InvalidItem:
    ...
    // ── Runtime preview rendering (mirrors FishingdexManager.RenderFish) ─────
    ...
    RenderTexture RenderItem(ShopItem item, int w, int h)
    {
        ...
        // Strip physics + colliders + scripts that might tick — preview is visual only.
        ...
        // Auto-frame: compute the combined renderer bounds and fit the camera so the
        // model's bounding sphere just fills the FOV. This way prefabs with off-center
        // pivots (handle at the bottom, head at the top, etc.) don't render off-center
        // or zoomed out. previewCameraDistance becomes a zoom multiplier on top of
        // the auto-fit (1.0 = tight fit, 1.5 = zoomed out, 0.8 = zoomed in).
        Bounds bounds;
        ...
        // DestroyImmediate, NOT Destroy: regular Destroy is deferred to end-of-frame,
        // so a follow-up RenderItem call in the same frame would still see this
        // instance on the preview stage and bleed it into the next item's preview.
