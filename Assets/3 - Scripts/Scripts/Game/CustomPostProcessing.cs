...
public class CustomPostProcessing : MonoBehaviour {
	...
	void Awake () {
		// Atmosphere shader requires depth texture; must be explicitly requested in builds
	...
	void OnRenderImage (RenderTexture intialSource, RenderTexture finalDestination) {
		...
		// Cache the bloom toggle once per OnRenderImage — checked per effect
		// below. CameraEffectsManager may not exist yet on the very first
		// frame after scene load, so we default to bloom-enabled when missing.
		bool skipBloom = false;
		{
			var cem = CameraEffectsManager.Instance;
			if (cem != null && cem.Input != null && !cem.Input.fxBloom) skipBloom = true;
		}
		...
		if (effects != null) {
			for (int i = 0; i < effects.Length; i++) {
				...
				if (effect != null) {
					// Diagnostic skip — pass-through Bloom when fxBloom is off
					// so the user can A/B "is the brightness flicker from bloom
					// catching the Sun mesh?". Skipping means currentSource is
					// preserved into the next effect's input (or final dest).
					if (skipBloom && effect is BloomEffect) continue;
