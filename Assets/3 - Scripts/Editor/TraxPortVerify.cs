...
/// If the two engines disagree by even one bit, the same track produces a
/// different loop — so a cassette printed in one build would sound wrong in the
/// next, and the "the same track always makes the same music" promise the whole
/// cassette economy rests on would quietly be false.
///
...
/// implementations, so the four Pow-derived audio values use a relative
/// tolerance — none of them feed a pattern decision.
///
/// A case is a whole TRACK: dials, key, six presets and six variations.
///
/// Regenerate after ANY change to prototypes/shuttle-computer/engine/:
