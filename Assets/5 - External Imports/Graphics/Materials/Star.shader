...
{
	// These textures are assigned at runtime by StarTest.Init / StarTest.Set,
	// but Unity rejects Material.SetTexture for properties that aren't
	// declared here — without this block the SetTexture calls fail silently
	// and the samplers return undefined/white textures, which makes the
	// shader compute starBrightness = 0 and renders the stars invisible.
	// Defaults are chosen so a freshly-loaded material still shows stars
	// at full brightness if Init hasn't run yet (black background, white
	// spectrum, no ocean mask).
	Properties
	{
		_MainTex    ("Background (set at runtime)", 2D) = "black" {}
		_Spectrum   ("Spectrum (set at runtime)",   2D) = "white" {}
		_OceanMask  ("Ocean Mask (set at runtime)", 2D) = "black" {}
		daytimeFade ("Daytime Fade",                Float) = 4
	}
	SubShader
	{
