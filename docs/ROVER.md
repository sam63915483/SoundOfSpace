...
| Space | tap = hop; hold = the suspension squats, release = bigger jump |
| Mouse | look around (clamped to ±150° so you can't look through your own head) |
| V | chase camera |
| F | get out (beside the driver's door) |
...
- Bounce: `springFrequencyHz` (softer = lower), `dampingRatio` (lower = bouncier).
