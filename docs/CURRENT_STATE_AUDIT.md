...
- **`Player/FireflyGlow.cs`** (auto-singleton, seeded) — the 60 s status: HDR emission written per suit slot through `SuitTinter.SetEmission` (property block, same block as the tint) + a point light on the player (culling mask excludes `PlayerReflect`; grass marker 0.5). **`Suit.mat` now has `_EMISSION` on with black emission and `m_LightmapFlags` 0** (the Standard inspector strips the keyword if the flags say EmissiveIsBlack) — zero visual change until FireflyGlow writes a colour. One slot, refresh-never-add, not saved, cleared in `NewGameReset`. `UI/FireflyGlowHUD.cs` = CatPerkHUD clone in amber, steps under the cat chip when both run.
