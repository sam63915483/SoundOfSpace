...
**Space free-float (NEW 2026-09-15, Sam):** the per-tick "align `transform.up` to the nearest body" write is SKIPPED while `_playerInSpace` (past `AtmosphereBounds.Radius(referenceBody)`, 8% hysteresis in `UpdateSpaceGate`) AND airborne AND not in the shuttle-proximity zone / intro up-override / rider mode / flat interior. Free: mouse-Y pitches the BODY (`freePitch`, unclamped), Q/E + pad LB/RB roll it (`spaceRollDegreesPerSecond`, gated on `TutorialAbility.Boost`), yaw as before; all three go through the same SmoothDamp + "applied" marker pipeline as yaw, and `CameraTransformFX` adds the pending pitch/roll remainder at render rate like it does yaw. Crossing outward folds the camera pitch into the body over `spacePitchFoldSeconds` (view never jumps). Re-entry sets `_gravityUpBlending` with `_smoothedGravityUp` seeded from the current up, so the feet swing back to the planet over `shipUpBlendSeconds`. `PlayerController.FreeFloating` (static) suppresses the E flashlight toggles in `LightToggle` / `PlayerFlashlight`. Per-planet tuning knob: `CelestialBody.atmosphereLineMultiplier` (0 = the visual haze edge, `(1+atmosphereScale)×radius`; airless bodies 1.5×radius) read by `AtmosphereBounds`. Spec: `docs/superpowers/specs/2026-09-15-space-free-float-design.md`.

**Movement model (2026-07 momentum/tunneling rework):** GROUNDED walk/run is `rb.MovePosition` displacement driven by `smoothVelocity` (SmoothDamp of input); AIRBORNE movement is pure `rb.velocity`. On leaving the ground (jump or ledge) the walk velocity is transferred into `rb.velocity` (`ForceMode.VelocityChange`), so horizontal momentum persists exactly like jetpack thrust; on landing, the tangential momentum (clamped to `runSpeed`) is seeded back into the walk pipeline and removed from `rb.velocity` (excess bleeds via the grounded-grip `momentumFriction` slide). Airborne WASD is Quake-style air-accelerate (`airAcceleration`, non-serialized, 24 m/s²): accelerates the frame-relative velocity toward input, capped at `walkSpeed` along the input direction — release keys = coast, hold back = brake, no free speed above the launch speed. The frame is the nearby orbiting ship when the proximity assist is engaged, else the reference body. Anti-tunneling is three layers: `ResolveWallSlide` (iterated `SweepTest` clamp+slide of the walk move), `ClampVelocityAgainstWalls` (speculative-contact cap of the into-wall `rb.velocity` component each tick), and `ResolveWallOverlap` (`ComputePenetration` rescue for the already-overlapping case where sweeps are blind — the old far-side-ejection failure). Airborne MovePosition teleports are gone, so CCD covers the flight path.

**`FallDamage.cs`** (`Assets/3 - Scripts/Survival/`) deals tiered landing damage from the max toward-surface speed in the last **0.25 s before touchdown** (32-slot ring sampled per FixedUpdate), not the all-time airborne peak — so thrust-braking before landing is safe, and touching water clears the window (`PlayerController.IsInWater`) so wading ashore after a cliff dive into water deals nothing. `FallDamage.Suppressed` is the cinematic mute (pod arrival).
...
**True-perspective corner screens (2026-07-11):** the art's painted pod screens taper toward a vanishing point, which overlay-canvas rotation (affine) can't match. With `HelmetHudConfig.perspectiveScreens` (default on), each corner cluster's card reparents into a `HudScreenProjector` capture rig — a world-space canvas + ortho camera parked 100k units above the main camera (GForceHUD thrust-widget keep-out trick, additionally scaled ×0.01 so it's sub-pixel from any frustum) rendering to a RenderTexture — and is re-drawn on its overlay canvas by `HousingScreenWarp`, a subdivided-grid Graphic that maps the RT through a square→quad homography onto the painted screen's measured corner points (`blQuad*`/`brQuad*` on the config, texture px; measured by edge-line fits on the art's dark glass). Corners are stored as canvas fractions so they ride the stretched art at any aspect ratio; the warp quads sway at gain 1.0 (they must track the frame exactly). Toggling `perspectiveScreens` off reverts to the legacy flat `SeatInArtHousing` path. The compass keeps flat seating with its own `browContentScale`/`browContentOffset` knobs. Trap: a runtime-added `Graphic` subclass does NOT inherit `RequireComponent(CanvasRenderer)` — create its GameObject with an explicit `CanvasRenderer` or it silently renders nothing.
...
### §16.5 Fireflies — catch, hold, eat, glow (NEW 2026-09-11, playtest pending)
Sam's idea. Spec `docs/superpowers/specs/2026-09-11-fireflies-design.md`, checklist `docs/PLAYTEST_FIREFLIES.md`. No scene wiring — everything is an auto-singleton or a Resources asset.
- **`World/FireflySpawner.cs`** (auto-singleton, seeded) — CatSpawner-shaped cube-face cell hash, but a cell is a **swarm** (3–5 bugs on evenly spaced home points over a 16 m patch, each wandering ≤ 3.5 m from its home — spread-out coverage, not clumps; `World/FireflySwarm.cs` root parented to the body via `ParentToBodyPhysicsFrame`, +Y = radial up, all flight in that local frame) and cells are **night-gated** by `FishingSun.SunDot` (spawn below −0.05, fade out above +0.02). Cap 36 swarms / 140 m, cell 40 m, chance 0.75 — density binds before the cap. Fully-caught cell rests 8 min (session only). The **12 nearest** bugs (within 60 m) carry a real point light + `GrassPointLight` (0.5 = ground parity); the rest glow only — a real light is one extra draw of the whole (single-mesh) planet, so `maxLitBugs` is a knob with a price. `InstancedGrassRenderer.GrassMaxPointLights` + the shader's `GRASS_MAX_POINT_LIGHTS` went 16 → 32 so fireflies and lanterns share the pool. Elects ONE `FireflyBug.Focused` per frame (nearest to the crosshair within `catchRange` 3.5 m) — the only bug that may prompt or be caught. `FeatureVault.Fireflies` gates the spawner only.
- **`World/FireflyBug.cs`** — an `Interactable` with NO colliders (gaze via mesh silhouette, in-range via one distance check; `gazeTarget` = the Body renderer so the 0.5 m halo quad is never outlined). `World/FireflyVisual.cs` is the single recipe for the bug's look (procedural ellipsoid + wing quads that flap in the vertex shader at 18 Hz + billboard halo; `SoundOfSpace/Firefly` shader, GPU-instanced with per-instance blink phase/period/floor/gaze/fade computed from `_Time` — no per-frame property blocks; `FireflyVisual.Blink()` mirrors the curve for the point lights). Materials are REAL assets `Resources/FireflyBody.mat` (opaque, q2000) + `FireflyHalo.mat` (additive, q3000 = after the atmosphere post, so the glow is never dimmed). Tail colour is HDR (~4×) so the atmosphere post's HDR exemption keeps it bright at night and bloom picks it up.
- **Hotbar:** `ItemId.Firefly` (appended), stack 10, resource path, procedural icon `FireflyIcon()` (override: `Resources/HotbarIcons/TransparentFirefly`). Eating = the existing hold-fire ring (`TickEatHold` → `ConsumeEquippedFirefly` → `FireflyGlow.Grant()` + burp). Held: `HeldItemViewmodel.BuildHeldFireflyVisual` (steady glow, point light 1.6 / 7 m, grass marker 0.5); `HeldItemResolver` uses the same builder so co-op partners see it.
- **`Player/FireflyGlow.cs`** (auto-singleton, seeded) — the 60 s status: a **glow shell** per body renderer (clone with the same mesh + bones, PlayerShadowProxy's trick, on layer Default so the proxy never shadows it) drawn with `Player/FireflyGlowShell.shader` (`Resources/FireflyGlowShell.mat`: additive, unlit, fresnel rim, HDR, `Offset -1,-1` so it wins the depth test against the surface it copies; per-slot `_Strength` — visor 0, `Suit Dark` 0.7) + a point light on the player (culling mask excludes `PlayerReflect`; grass marker 0.5). **⚠️ The astronaut's materials are EMBEDDED in `Astronaut.fbx`** (`materialLocation` = InPrefab) — `Astronaut Mat/Suit.mat` on disk is NOT used by the model, and an embedded material can't get `_EMISSION` switched on, which is why the first (emission) version glowed for nobody. One slot, refresh-never-add, not saved, cleared in `NewGameReset`. `UI/FireflyGlowHUD.cs` = CatPerkHUD clone in amber, steps under the cat chip when both run.
...
**Third-person view (NEW 2026-09-15, experiment Sam wants kept).** `V` cycles `CameraTransformFX.ViewMode` 0 → 1 (close, `closeOffset` 2.5 m back) → 2 (far, `farOffset` 6 m) → 0; the offset blends (`viewBlendSpeed`), orbits the head by the look rotation, sphere-casts against `PlayerController.walkableMask` so it never enters the planet/ship, and aims `lookAhead` metres past the head. Not saved; ignored while piloting / SolarMap / pool. Works with camera effects switched off. Two structural consequences:
- **`CameraTransformFX.Eye`** — a `FirstPersonEye` transform under the player, posed every LateUpdate at the *first-person* camera pose. `EnsureEye` moves every non-Camera/non-Canvas child of the camera (`CameraHoldPos` + held-item rigs, torch, viewmodel fill light) under it, so hands stay on the head while the camera swings out. Anything that needs "the camera the hold point hangs under" calls `CameraTransformFX.ViewFrameOf(t)` (pistol/grapple ADS, beer cup, axe swing wall probes, blade sweep, `HeldItemViewmodel`, `HeldItemResolver.TryGetViewGeometry`, `NetworkAvatarDetail` head pitch). In first person Eye == camera, so nothing changed there.
- **`PlayerAimCast`** (`Camera/PlayerAimCast.cs`) — Raycast/SphereCast wrappers that return the nearest hit NOT under the player root, plus `ExtraReach` (= `CameraToEyeDistance`). Used by `InteractGaze`, pistol, axe, grapple, `PlayerPickup`, so the crosshair cast from a chase camera lands on what the dot is on instead of the astronaut's back, and short reaches aren't shortened.

**Jetpack thruster flames (NEW 2026-09-15).** `Player/JetpackThrusters.cs`, auto-attached to the PlayerController by a `sceneLoaded` hook (build-safe). Any transform under the player named `Thruster*` is a nozzle; its +Z is the exhaust direction; a procedural additive particle jet is built under it. Firing is geometric: linear when `-exhaust · PlayerController.ThrustLocal` agrees (up/down/directional boosts, circularize), RCS in free-float when `(r × -exhaust) · PlayerController.FreeLookRateDeg` agrees (mouse pitch/yaw, Q/E roll). Name words: `Main` = linear only, `RCS` = rotation only. `[ContextMenu] Rescan` after adding nozzles in Play mode (also auto-rescans every 2 s while it has none).
...
**Tutorial box (2026-09-14, phase 1 — spec `docs/superpowers/specs/2026-09-14-tutorial-box-design.md`).** A third destination scene, `Assets/4 - Scenes/Tutorial.unity`, reached from the main menu's **TUTORIAL** row (`MainMenuController.OnTutorial`, same waist as PLAY: menu FX manager destroyed, `LoadingScreen` + `EnsureGameplaySingletonsAsync`, so the whole HUD stack is live in a build). Built by `Tools ▸ Solar System ▸ Build Tutorial Scene` (`Editor/TutorialSceneBuilder.cs`, additive create → save → close; regenerating wipes hand-placed extras). Contents: a 200 m box — `Tutorial Ground` = a **fake planet** (kinematic `Rigidbody` + `CelestialBody` radius 10 km, centre 10 km below the floor, no generator) so `ShuttleAutopilot` has a parent body to fly against; **no `NBodySimulation`** on purpose (that is what switches `PlayerController` to its flat-floor gravity — a sim with zero bodies leaves the player in zero-g); floor = green slab, 4 walls + ceiling = quads with `Custom/TutorialDigitRain` (procedural falling 0s/1s, see-through to the Milky Way skybox), all on layer 10; `Shuttle_Lander` prefab instance (feet on the floor; its authored pose is the landing target); `Player.prefab` with the gameplay overrides (`walkableMask` 34304, camera Skybox, `Planet Effects` stripped from the post list) standing in the pod. **`TutorialDirector`** runs the cut-down wake: `NewGameReset.Apply()` (fresh state, no seed-save runner), `PrepareIntroApproach(185 m, straightIn: true)` (new optional params on the autopilot; defaults = real intro), player into the pod, `CaptureIntroRiders`, `LaunchIntroApproach(12 s)`, door `OpenHold` after 1 s, one `TutorialUI` hint at the hover, hidden on `Parked`. Everything after launch is the real travel code (hover, NAV feed, F/WASD/QE/SPACE landing, ramp). **`TutorialSession.IsActive`** (active scene == "Tutorial") fences the file writers — `StasisPodSave` plays DOWNLOADING only, `DeathCutsceneController` respawns in place, pause menu hides MULTIPLAYER — so **the tutorial never writes a save**. **Round 2 (same day):** the scene now HAS an `NBodySimulation` (both bodies pinned) plus a real Sun body 25 km out (directional light + `SunShadowCaster` + point light + emissive ball) so the lens flare, grass, cats and gravity (`surfaceGravity` 8 = Humble Abode) work as in the game; the director vents a 55 % O2 reserve (`PlanetOxygen.AddVentedReserve`). Panes carry `LensFlarePassThrough` (new marker the flare's occlusion test skips). Props: `TutorialPropField` places trees/crystals/mushrooms once at load (live spawners scan the whole 10 km sphere per tick); `InstancedGrassRenderer` + `CatSpawner` (cap 5) run live; floor = MeshCollider named `Terrain Mesh`. Crosshair (`CrosshairReticle` on a `Dot`) is built by the builder; `HelmetHudConfig` comes from a prefab snapshot (`Tools ▸ Solar System ▸ Snapshot HelmetHudConfig Prefab`). Fixed for the whole game: `LensFlareRegistry.OnDisable` hides the flare (it froze on screen), `CameraEffectsManager`/`TabbedPauseMenu` fall back to `InputSettings.Active` when no `SettingsMenu` exists, and the top-left `ShuttleAutopilot.OnGUI` diagnostic is opt-in (`DebugOverlay`, F12). **Round 5:** radius 750; the spot search requires water (10–45 % of a 9×9 box grid, ≥3 m under sea), a flat dry ±30 m centre and a ≥45 m hill, relaxing hill→water if needed. 🔥 `InstancedGrassRenderer.maxHeightAboveWater` / `waterMargin` are METRES above sea level (1…15 on HA) — on a k× planet the band is a shoreline strip; the tutorial scales both by k. Ceiling digit material has `_Radial` = 1 (streams out from the centre). **Space dust on the GPU (2026-09-14 late):** `SpaceDustField.GpuPath` (default on; F2 cheats = A/B, state carried across). `Resources/SpaceDustStep.compute` is a line-for-line port of the per-speck loop (drift, density, edge/twinkle, ocean segment test, co-move/parallax, wrap) writing `_DustColor` + appending visible ids; one `DrawMeshInstancedIndirect`; `SpaceDust.shader` gained `procedural:setupDust` (target 4.5). Motes are purely visual (collection = SpaceNet, not motes) and additive, so order-free. `FrustumCull` static exists but is OFF (the CPU path never culled; secondary cameras see the same field). Pre-existing oddity left alone: the material's custom queue 3000 overrides the shader's 2450. **GPU-resident grass — PLAYTESTED (Sam, build: tutorial 230 vs 130 fps at 1.8×, real save 8-9 → ~6 ms); the gameplay scene's grass object is switched on too (`gpuResident: 1` + `cullCompute`). Plus `residentWholePlanet` (default on; needs a baked blob): all ~150 k baked blades uploaded once, no CPU streaming, blades grow in by a distance curve (`residentNearRadius` 60 / `residentFarRadius` 350 / `residentFalloff` 2 / `residentGrowBand` 0.08, × GRASS DISTANCE) — no pop-in. Previous note:** **GPU-resident grass (2026-09-14 evening, TRIAL in the tutorial only):** `InstancedGrassRenderer.gpuResident` + `cullCompute` (`World/GrassCull.compute`) keep blades in a planet-local StructuredBuffer (slot per cell, dirty-run uploads), cull/thin on the GPU into per-mesh append buffers, and draw with `DrawMeshInstancedIndirect` for both the colour pass and the depth pre-pass from the same args — the two passes can't disagree. Shaders `CG_SimpleGrass`/`CG_GrassDepth` carry `procedural:setupGrass…` (target 4.5); `CG_SimpleGrass` is now Always Included. F11 (cheats) = `ForceCpuPath` A/B. Gameplay scene still on the CPU draw. **Round 4 (2026-09-14, the current shape):** the slab and fake body are GONE. The tutorial scene has a **real generated planet** — Humble Abode's Shape/Shading/Atmosphere/Ocean cloned into `Solar System/Tutorial Earth/` (data copies, generation code untouched), `CelestialBody` r=1500 g=8 pinned, named **"Humble Abode"** (name-keyed systems: grass `onlyBodyName`, OxygenManager refill zone), `Mesh Holder` BodyPlaceholder, `waterline` r=1500, plus the scene-level `SolarSystemSpawner` (300/100/50) + `LODHandler` the generator needs. The builder samples `shape.CalculateHeights` (read-only) to find the flattest dry 350 m patch and rotates/places the planet so it sits under the axis-aligned box at y=0. Spawners are prefab SNAPSHOTS of the gameplay objects (`TutorialSnapshots.SnapshotSpawners` → `1 - samsPrefabs/TutorialSpawners/`), grass streams live, cats capped at 8. Box 350 m (walls 450 tall from y=−100). `LetterboxBars` ignores `IntroSequenceController.ShuttleWakeActive` (the pod-phase movement pin is not dialogue). `TutorialPropField` deleted. **Round 3:** `FireflySpawner` is quiet in the tutorial (its cubeface scan is radius² — 1.5 M cells/tick on the 10 km body = 5 fps). `LoadingScreen` now holds 0.5 s at 100 % over the live scene and fades out over 1 s (`HoldSeconds`/`FadeSeconds`, `IsHolding` for directors to stage under). **Round 3b:** the tutorial player is `Assets/1 - samsPrefabs/TutorialPlayer.prefab`, a prefab VARIANT snapshot of the gameplay scene's (inactive-until-GameSetUp) Player instance — audio clips + all ~20 scene-added components come along (`Tools ▸ Solar System ▸ Snapshot Tutorial Player Prefab`, `Editor/TutorialSnapshots.cs`, same tool as the helmet config). `StasisPodSave`: the DOWNLOADING half of the ritual is gone game-wide (heal + unlock + open door, no overlay); UPLOADING unchanged.
...
| Fireflies (2026-09-11) | `FireflySpawner` (World/), `FireflyGlow` (Player/), `FireflyGlowHUD` (UI/) — §16.5 |
...
## Addendum 2026-09-15 — pod door, grass, sky, release hold, trees

- **Pod door timing (tutorial / new game / load):** opens 1 s AFTER the loading cover has fully faded. `TutorialDirector` waits `!LoadingScreen.IsShowing` + `doorOpenDelay`; `IntroSequenceController.podDoorDelayAfterEyesOpen` (1 s after the eyes finish opening, was 3 s) and wake clicks don't count under the cover; `StasisPodSave` does nothing while the cover is up (its frame-1 "sealed Deep = load-wake" was the real early opener) and its load-wake waits cover-gone + `wakeDoorDelay`. While the pod is shut the movement pin (`isInDialogue`) no longer blocks LOOK or shows the dialogue vignette during `ShuttleWakeActive` (`PlayerController`, `CameraEffectsManager`).
- **Tutorial grass = resident like Humble Abode:** `Tools ▸ Solar System ▸ Bake Tutorial Grass` (`Editor/TutorialGrassBake.cs`) bakes the box area only (`InstancedGrassRenderer.EditorBake` keepCentre/keepRadius) to `Tutorial Earth/Tutorial Earth Grass.bytes`; the builder assigns it on rebuild; the tool never saves the scene. Runs the edit-mode generator by reflection (an unfocused editor never ticks `[ExecuteInEditMode]` Update).
- **🔥 GPU grass rule:** never centre an indirect draw's `Bounds` on the viewer — Unity uses bounds.center as the object position for passes without the procedural setup, which pinned a phantom depth-only blade cluster to the player. Bounds are now planet-centred.
