...
3. The recurring gamebreaking bug class here is the **promise/grade mismatch**: one system displays or promises X while another grades Y. Three shipped examples are described in §9. Anything in this class outranks balance concerns.
...
With the skew, roughly 55% of aliens sit in the bottom quarter of the pickiness band (broad, cheap) and ~19% in the top third (fussy, premium).
...
At the starting 2 plugins, `Base` = $14 (Type 1). A tolerant cheap buyer pays roughly $8–9 for a decent tape; a bonded fussy superfan with a bullseye tape reaches $40+ before haggling. Negotiation numbers derived from `value`: the buyer's *opening thought* is a slight lowball; their *ceiling* is `value × Patience`; a "final offer" figure exists in the model (see §9, trap 6 — it never actually fires in-game).
...
2. Player replies via chips (no free text): **ACCEPT** → pick a meetup window (5 min "+15%", 10 min "+10%", 15 min "+5%" gratitude labels) → deal `Scheduled` at the quoted price. **COUNTER** → one price slider (up to ~2× quote); the buyer accepts (≤ their patience ceiling), counters back a number ("TAKE 23"), or refuses. **NOT NOW** → declined.
3. The agreement stores: genre, qty (always 1 for tapes currently), agreed price-per-tape, window, deadline. Missing the deadline halves bond and sends a "you never showed" text.
