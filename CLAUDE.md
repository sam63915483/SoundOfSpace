...
- **…and its corollary — glass you LOOK THROUGH must be queue > 2500 (the opposite rule).** The ≤2500 rule above is for things you look AT. A pane at ≤2500 lands in `_CameraDepthTexture`, and the atmosphere post does `dstThroughAtmosphere = min(hitInfo.y, min(sceneDepth, dstToOcean) - dstToAtmosphere); if (>0) scatter;` — so a pane at arm's length makes `sceneDepth ≈ 0`, the branch is skipped, and those pixels keep their **raw** colour. Symptom: through the window the sky is dark by day (the blue *is* the post — there's no blue skybox), and by night the ground looks grey and *lit*, because the post is also what multiplies the night side down. **`ZWrite Off` and `Cast Shadows = Off` are NOT sufficient — both were tried and failed.** Queue 3000 is what works: `Custom/WindowGlass` (village), `Shuttle_PodGlass.mat`, `StasisPodGlass.shader`. `Custom/AtmosphereGlass` (queue 2450) is only for panes you look at, e.g. the space net.
